...
const stripePromise = loadStripe('pk_test_51Qfrj02MOn24WkqKn76NRmUzZqVRVfMOjoNjDGg1ek4ESwHf9hfiC06vVzULcyQDbrDMxhe8YvIrIcxp96X4sIYf00dp7gdIlA');
...
const Cart = () => {
    ...
    if (cartItems.length === 0) {
        return <div className='text-black text-3xl text-center my-72'>Sepetiniz boş :( </div>;
    }
  ...
  return (
    <div className='p-4 mt-16 max-w-[1400px] mx-auto'>
        <h2 className='text-2xl font-semibold text-center my-6'>Alışveriş sepeti</h2>
        ...
        <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-6'>
            {cartItems.map((item, index) => (
                <div key={index} className='bg-white rounded-lg shadow-lg p-4 flex flex-col'>
                    ...
                    <p className='text-md mb-1'>Fiyat: {(item.priceInCents / 100).toFixed(2)} TL </p>
        ...
        <div className='text-center mt-8'>
            <p className='text-2xl font-semibold mb-4'>Toplam Fiyat: {(totalPrice / 100).toFixed(2)} TL </p>
            <button onClick={handleCheckout} className='text-black text-xl font-bold py-4 px-8 rounded-xl bg-yellow-400 hover:bg-yellow-500'>

                Ödemeye geçin</button>
        </div>  
    </div>
  )
}
